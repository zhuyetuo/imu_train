...
| [skin_health/docs/skin_health.md](skin_health/docs/skin_health.md) | 狗狗皮肤健康评估任务：SBS评分引擎、分层贝叶斯模型（有序标签版+抓挠计数版），代码/文档/数据都在 [skin_health/](skin_health/) 目录下，见该目录的 [README](skin_health/README.md) |
...
**结论先说：狗场夜里那几路单间，画面判不出"在不在抓挠"，该去补红外补光，不是接着调算法。**

这是 2026-09-22 在任务 #45715（IMU19，`00:37:24.705` 那条 82% 的抓挠候选）上量出来的，
不是估计：

| 事实 | 数值 |
| --- | --- |
| 原片实际用到的亮度范围 | **14~40，一共 26 级**（满量程 256 级） |
| 要铺满就得放大 | **9.81×** |
| 30 帧对齐平均后信噪比涨 | **×5.5**（按 √N，这是理论上限） |

26 级动态范围意味着传感器**本来就没记录下更多信息**。软件能做的只是把已有的信息摆得
更清楚——堆栈、拉伸、去色噪都是只减不加的运算，变不出没拍到的东西。

### 能用的（`vision_service/lowlight.py`）

- **多帧堆栈**：噪声每帧随机、机位固定，对齐后平均 30 帧。**一个像素都不编造**，
  这是软件能做到的上限。看得出"狗侧卧在地上、头朝哪边"。
- **整段循环播放**（`enhance_seq`）：抓挠是动作，单帧判不出来。全段共用一套拉伸
  映射（按帧各算各的会亮度乱跳），3 帧滑动平均 + 双边滤波。看得出"有没有在动"。
- **去色噪**：这个亮度下色度通道全是读出噪声，放大后满屏紫麻点盖住轮廓。压掉之后
  亮度一个字节没动（有测试钉住）。

### 不能用的：深度学习夜视增强

**Retinexformer 这条路在这批素材上是死的**，前后试了两种输入，都不行：

1. 喂原始暗图（14~40 级）→ 糊成一团，还染红。
2. 喂堆栈+拉伸+去色噪之后的图（已经能看见狗了）→ **输出成块的橙绿紫**，纯粹在编。

权重不是选错的：用的是 `SMID.pth`，正是拿真实低光**视频**训的那个（LOL_v1/v2 是
照片数据集，一开始就排除了）。问题在于 SMID 训练时见过的低光场景跟这个近黑单间
差太远。而且它 256×256 跑一次 358ms，做不成播放器上的实时滤镜（要 30fps，差三个
数量级）。

代码留着（`run_model`、模型服务里那一行、弹窗里「也跑一下模型」那个按钮），**但默认
