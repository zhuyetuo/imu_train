...
# 出力，吞吐是相加的，不是抢。
#
# 2026-09-21 实测三种配置，每路耗时：
#
#   6 路全 NVDEC              12.6 s
#   12 路全 NVDEC             14.8 s   ← 最差：2 个 NVDEC 引擎排队 + 12×600 MiB 上下文
#   12 路 = 6 NVDEC + 6 软解  11.9 s   ← 只比基线好 6%
#
# 差别都在几个点以内，说明瓶颈已经不在解码并行度上了——抠狗/向量/姿态那几步在
# 服务里是**加锁串行**的（单卡并发只买到显存翻倍和碎片化），并发再高它们照样排队。
#
# **定在 0.5（6 路 NVDEC + 6 路软解，配 web 机 VISION_INDEX_CONCURRENCY=12）。**
# 这是量过的三组里唯一最好的那个：
#
#   6 路全 NVDEC              12.6 s/路
#   12 路全 NVDEC             14.8 s/路
#   12 路 = 6 NVDEC + 6 软解  11.9 s/路   ← 这个
#
# 别的比例（0.667 + 并发 18 之类）没量过就别当默认。差别本来就只有几个点——
# 瓶颈已经不在解码并行度上了，抠狗/向量/姿态在服务里是加锁串行的，
# 解码再快这几步照样排队。
DECODE_CPU_SHARE = float(_env("DECODE_CPU_SHARE", "0.5"))
# 软解每路给几个线程。不限制的话 ffmpeg 默认按核数开，十几路一起就是几百个线程
# 互相抢，比单路还慢。
# 6 路软解 × 4 = 24 线程，14900K 有 32 个——剩下 8 个给裁图/帧差/JPEG 那些活，
# 以及平台后端自己。**11.9 s/路 那次就是在 4 下量的**，改这个数就不是同一个配置了
