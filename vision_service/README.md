...
### 一句话搜：实测在这批素材上不好使（2026-09-21）

**别再花时间调措辞了。** 同一句 `a dog licking its hind leg`，三个变量各试一遍：

| 试的 | 最高分 | 命中 |
|---|---|---|
| 抠图那一列（原来的） | 0.211 | 杂 |
| 换成没抠背景那一列（`EMBED_RAW_TOO`） | 0.198 | 没变 |
| 再关掉去共同背景 | **0.134** | **更差**：塌成同一路的连续帧 |

三次都没动。结论：**SigLIP-base 在俯拍监控画面上对"动作类"文字没有分辨力。**
它能分场景（房间、地板、笼子）和大致姿态（趴着/站着），但"舔后腿"这种由细微
肢体关系定义的东西，224×224 的 base 模型在小目标俯拍下分不出来——跟措辞、
跟抠不抠背景、跟减不减均值都无关。

**以图搜图不受这个结论影响**（同一批素材上 0.74~0.88），因为两边都是图、同分布。

这张表还有个副作用：**两列打出来的分没有可分辨的差别**（0.211 vs 0.198）。所以
老索引（没有 `emb_raw` 那一列）一句话搜时**退回抠图那一列照搜**，不再整路跳过——
原来跳过的理由是"两个空间的分数不可比"，那是猜的，量出来的结果是一样的。
为一个量不到的好处把十几路素材扔在外面，人看到的是"搜不到"，而素材就在那儿。
退回的路数照实报（`old_index`）。

所以一句话搜只当"碰运气的辅助入口"，第一阶段的主力是「大模型看视频找动作」。
真要救它，只有换更强的图文模型（SigLIP2-large / EVA-CLIP）——那要重建全部索引、
吃更多显存，**动手之前先拿一路视频验**：同一句话在新旧模型上各搜一次，数前 20 张
里对几张。没有这个数就别开工。
