...
- **解码是建索引最大的一笔（实测约六成）**，而且一路视频只能串行解。2026-09-21
  ...
  （抠狗/向量/姿态在服务里是加锁串行的，并发再高照样排队）。现在默认在试
  **6 硬解 + 12 软解**：`DECODE_CPU_SHARE=0.667` + 平台 `VISION_INDEX_CONCURRENCY=18`
  + `DECODE_CPU_THREADS=2`（12×2=24 线程，14900K 有 32 个，留 8 个给别的活）。
  **这几个是配套的，只改一个必然更差。** 已知可用的两格：`share=0` + 并发 8；
  `share=0.5` + 并发 12（实测 11.9 s/路）。
