...
只读索引里的 meta，不解码视频、不跑模型，几秒出结果。**覆盖率 = 有姿态 / 有狗**
（没狗的帧本来就不该有姿态，拿采样数当分母会把它压得虚低）。公共区 cam7 跟单间
机位分开看：视角差得远，一个的覆盖率说明不了另一个。
