...
ensure_deps() {
    ...
    for pair in "fastapi:fastapi" "uvicorn:uvicorn" "numpy:numpy" "cv2:opencv-python-headless" "ultralytics:ultralytics" "anthropic:anthropic" "httpx:httpx" "transformers:transformers" "PIL:pillow" "sentencepiece:sentencepiece"; do
        local mod="${pair%%:*}"
