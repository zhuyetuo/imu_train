...
## 疑似舔/啃候选（只靠加速度，给「舔/啃哪个部位」攒标注）

稳定版/v2 的 `candidates` 里现在还会混着 `reason="grooming"`、`label="舔身体"` 的候选（`GROOM_ENABLED=1` 默认开）。它**不靠模型**，只看三轴加速度：

1. **姿态偏离基准** —— 舔/啃时舌头和下颌的力传不到脖子，真正到项圈的是「头必须伸到那个部位并保持住」，重力方向明显偏离这只狗平时的姿态（`GROOM_TILT_MIN_DEG`，默认 25°）。基准是**这份文件里最安静的窗口**的重力方向中位数，所以是相对本次佩戴的，项圈转了角度也不影响。
2. **动作量适中** —— 太小是趴着不动（`GROOM_MOTION_MIN`），太大是走/跑（`GROOM_MOTION_MAX`）。
3. 节律（1.5–4.5 Hz）只用来排序不卡门槛，慢舔也没有节律。

跟抓挠（正式片段 + 疑似抓挠）重叠的去掉——抓挠时头也歪，那段已经有人在看了。每个文件最多 `GROOM_MAX` 条。这是**候选**不是判断：站着嗅地、低头喝水会被挑进来，靠人排除，排除记录本身就是训练时的负样本。

平台那边按 `label` 找标签（项目里得有「舔身体」这个标签），确认时可以改成别的类别——想攒**部位**标注，就在项目里建「舔身体-前肢」「舔身体-后躯」这样的标签，确认时从「改成别的」里选，不用改任何表结构。
...
- `GET /api/v1/label/queue`：几个在算、几个在等、平均一个文件多久、预计多久消化完。
  批量推理开始/结束的日志里也会带一份。
- 每个 worker 内部的 numpy / BLAS / OpenMP 限成**单线程**：并行度由进程池提供（默认核数 - 2 个进程），进程里再各自开满线程会有上千个线程抢几十个核，光上下文切换就吃掉大半算力。只限推理 worker，训练那边（XGBoost/LightGBM 靠 OpenMP 多线程）不受影响。
